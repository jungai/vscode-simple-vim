        ...
        (vimState, editor) => {
            execMotion(vimState, editor, ({ document, position }) => {
                ...
                return position.with({
                    character: Math.max(lineLength - 1, 0)
                });
            });
        }
    ),
...
function execMotion(
    ...
    const newSelections = editor.selections.map((selection, i) => {
        if (vimState.mode === Mode.Normal) {
            ...
            const vimSelection = vscodeToVimVisualSelection(
                document,
                selection
            );
            ...
            const vimSelection = vscodeToVimVisualLineSelection(
                document,
                selection
            );
            const motionPosition = motion({
